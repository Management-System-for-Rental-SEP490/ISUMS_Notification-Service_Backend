...
@RestController
@RequestMapping("/api/notifications/technician")
...
public class TechnicianNotificationController {
    ...
    public ResponseEntity<SseEmitter> stream(@AuthenticationPrincipal org.springframework.security.oauth2.jwt.Jwt jwt) {
    ...
    public ApiResponse<Map<String, Long>> unreadCount(@AuthenticationPrincipal org.springframework.security.oauth2.jwt.Jwt jwt) {
        UUID userId = UUID.fromString(jwt.getSubject());
    ...
    @PutMapping("/read-all")
    public ApiResponse<Void> markAllRead(@AuthenticationPrincipal org.springframework.security.oauth2.jwt.Jwt jwt) {
